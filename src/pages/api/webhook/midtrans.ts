...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    switch (transaction_status) {
      case 'capture':
      case 'settlement': newStatus = fraud_status === 'challenge' ? 'pending' : 'completed'; break;
      case 'pending': newStatus = 'pending'; break;
      case 'deny':
      case 'cancel':
      case 'expire': newStatus = 'cancelled'; break;
      default: newStatus = 'pending';
    }

    if (supabase) {
      await supabase.from('orders').update({ status: newStatus, updated_at: new Date().toISOString() }).eq('id', order_id);
      await supabase.from('transactions').insert({ order_id, midtrans_transaction_id: transaction_id, payment_type, gross_amount, status: transaction_status, fraud_status, raw_response: JSON.stringify(notification), created_at: new Date().toISOString() });
    }
